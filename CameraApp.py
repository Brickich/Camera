...
class CameraSettingsFrame(QFrame):
    ...
    def _initCameraTriggerLayout(self):
        ...
        self.triggerButton.setStyleSheet(
            "font-size: 16px; font-family: monospace; padding: 5px;"
        )
        ...
        self.triggerSourceCombobox = QComboBox()
        self.triggerSourceCombobox.addItems(triggerSources)
        self.triggerSourceCombobox.setMinimumSize(30, 30)
        self.triggerSourceCombobox.setCurrentText(self.camera.TriggerSource.get()[1])
        triggerSourceLayout.addWidget(self.triggerSourceCombobox)
        ...
        self.triggerActivationCombobox = QComboBox()
        self.triggerActivationCombobox.addItems(triggerActivations)
        self.triggerActivationCombobox.setCurrentText(self.camera.TriggerActivation.get()[1])
        triggerActivationLayout.addWidget(self.triggerActivationCombobox)
        self.triggerActivationCombobox.setMinimumSize(30, 30)
    ...
    def _initCameraControlLayout(self):
        ...
        self.playButtonIcons = {
            False: QIcon("assets/play_button.png"),
            True: QIcon("assets/pause_button.png"),
        }
        ...
        self.crosshairButton.clicked.connect(
            lambda checked: self.camera.switch_crosshair()
        )
        ...
        self.whiteBalanceButton.clicked.connect(
            lambda checked: self.camera.switch_white_balance()
        )
        ...
        self.rotateRightButton.clicked.connect(
            lambda checked: self.camera.rotate_image_right()
        )
        ...
        self.rotateLeftButton.clicked.connect(
            lambda checked: self.camera.rotate_image_left()
        )
        ...
        self.flipVerButton.clicked.connect(
            lambda checked: self.camera.flip_image_vertically()
        )
        ...
        self.flipHorButton.clicked.connect(
            lambda checked: self.camera.flip_image_horizontally()
        )
    ...
    def _initSliderLayout(self):
        for setting in self.camera.sliderSettings:
            if hasattr(self.camera, setting):
                ...
                try:
                    ...
                    if setting == "FramesQuantity":
                        slider_range = {"max": 1000, "min": 5, "inc": 1}
                ...
                slider.valueChanged.connect(
                    lambda val, ent=entry: self._onSliderChange(val, ent)
                )
                ...
                if setting in ["Width", "Height", "OffsetX", "OffsetY"]:
                    ...
                    slider.valueChanged.connect(
                        lambda value, stng=setting: self._onDependentSettingsChange(
                            value, stng
                        )
                    )
                ...
                entry.returnPressed.connect(
                    lambda sldr=slider, ntry=entry: self._onEntryChange(sldr, ntry)
                )
        ...
        controlLayout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        presetControlLayout.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.loadPresetButton = QPushButton("Load Preset")
        presetControlLayout.addWidget(self.loadPresetButton)

        self.savePresetButton = QPushButton("Save Preset")
        presetControlLayout.addWidget(self.savePresetButton)

        self.mainLayout.addLayout(controlLayout)
        self.mainLayout.addLayout(presetControlLayout)
    ...
    def getPreset(self):
        preset = {}
        triggerActivation = self.triggerActivationCombobox.currentText()
        triggerSource = self.triggerSourceCombobox.currentText()
        ...
        preset["TriggerSource"] = triggerSource
        preset["TriggerActivation"] = triggerActivation
        return preset
    
    def setPreset(self , preset):
        self.triggerActivationCombobox.setCurrentText(preset["TriggerActivation"])
        self.triggerSourceCombobox.setCurrentText(preset["TriggerSource"])
        for setting , sliderDict in self.settingsSlidersDict.items():
            value = preset[setting]
            for slider in sliderDict.keys():
...
class SettingsWindow(QWidget):
    def __init__(self, parent, cameras: list[Camera]):
        ...
        self._initCameraSettingsControlLayout()
    ...
    def _initCameraDisplayLayout(self):
        cameraDisplaySelectionLayout = QHBoxLayout()
        cameraDisplaySelectionLayout.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.allCameraDisplayButton = QPushButton("ALL")
        ...
        for i in range(len(self.cameras)):
            displayCameraButton = QPushButton(f"Cam {i + 1}")
    ...
    def _initCameraSettingsLayout(self):
        ...
        for i, camera in enumerate(self.cameras):
            showCameraSettingsButton = QPushButton("Cam {} Settings".format(i + 1))
        ...
        self.cameraSettingsSelectionGroup.buttonClicked.connect(
            self._onCameraSettingsClicked
        )

    def _initCameraSettingsControlLayout(self):
        cameraSettingsControlLayout = QHBoxLayout()
        cameraSettingsControlLayout.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.copyButton = QPushButton("Copy Settings")
        cameraSettingsControlLayout.addWidget(self.copyButton)

        self.mainLayout.addLayout(cameraSettingsControlLayout)
        

    def _initFrameViewerLayout(self):
        frameViewerLayout = QHBoxLayout()
...
class TriggerWorker(QThread):
    ...
    def _process_images(self):
        try:
            ...
            for i, rawImage in enumerate(self.camera.rawImages):
                if i > 0:
                    self.camera.timestamps.append(
                        int(rawImage.get_timestamp() - self.camera.timestamps[0]) / 1000
                        + delay
                    )
                else:
            ...
            self.imagesProcessed.emit(
                self.cameraIndex, self.camera.numpyImages, self.camera.timestamps
            )
    ...
    def startTrigger(self):
        try:
            ...
            self.camera.triggerSettings()
            triggerSource = self.camera.TriggerSource.get()[1]
...
class ImageWorker(QThread):
    ...
    def flipImage(self, pixmap: QPixmap):
        flipped_image = QImage(pixmap).mirrored(
            horizontal=self.camera.flipHorEnabled, vertical=self.camera.flipVerEnabled
        )
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        leftWidget = QWidget()
        leftWidget.setMinimumSize(width - 350, height)
        ...
        self.settingsWindow.allCameraDisplayButton.clicked.connect(
            lambda checked: self.imageLayout.setDisplayMode("All", 0)
        )
        self.settingsWindow.cameraDisplaySelectionGroup.buttonClicked.connect(
            lambda button: self.imageLayout.setDisplayMode(
                "Single", self.settingsWindow.cameraDisplaySelectionGroup.id(button)
            )
        )

        self.settingsWindow.frameViewerButton.clicked.connect(self._showFrameViewer)
        self.settingsWindow.copyButton.clicked.connect(self._copyButton)
    ...
    def _initCameraWorkers(self):
        for i, camera in enumerate(self.cameraControl.cameras):
            ...
            triggerWorker.workerEnd.connect(
                lambda idx=i: self._onTriggerWorkerFinished(idx)
            )
            triggerWorker.imagesProcessed.connect(
                lambda idx, imgs, tmstmps: self._omImagesProcessed(idx, imgs, tmstmps)
            )
            self.triggerWorkers.append(triggerWorker)

            settings_frame = self.settingsWindow.cameraSettingsFrames[i]
            settings_frame.playButton.clicked.connect(
                lambda checked, idx=i: self._toggleCameraRecording(checked, idx)
            )
            settings_frame.applyButton.clicked.connect(
                lambda checked, idx=i: self._applyCameraSettings(idx)
            )
            settings_frame.triggerButton.clicked.connect(
                lambda checked, idx=i: self._toggleCameraTrigger(checked, idx)
            )
            settings_frame.captureModeButtonGroup.buttonClicked.connect(
                lambda btn, idx=i: self._toggleCaptureMode(btn, idx)
            )
            settings_frame.loadPresetButton.clicked.connect(
                lambda btn , idx = i : self._loadCameraPreset(idx)
            )
            settings_frame.savePresetButton.clicked.connect(
                lambda btn , idx = i : self._saveCameraPreset(idx)
            )
    ...
    def startCameraTrigger(self, cameraIndex: int):
        ...
        if not success:
            self.settingsWindow.cameraSettingsFrames[
                cameraIndex
    ...
    def _omImagesProcessed(self, index: int, images: list, timestamps: list):
        if len(images) >= 5:
            ...
            QTimer.singleShot(
                100,
                lambda: [
                    self.frameViewer.update_display_for_camera(index),
    ...
    def _toggleCameraRecording(self, checked: bool, cameraIndex: int):
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to toggle camera capture: {str(e)}"
            )
    ...
    def _applyCameraSettings(self, cameraIndex: int):
        try:
            ...
            self.imageLayout.setCameraInfoLabel(cameraIndex)

            settingsPane.captureModeButtonGroup.button(1).setChecked(True)
            QMessageBox.information(self, "Success", f"Preset for camera {cameraIndex + 1} applied successfully!")
    ...
    def _copyButton(self):
        ...
        if  camerasCount > 1:
            preset = self.settingsWindow.cameraSettingsFrames[0].getPreset()
            self._applyCameraSettings(0)
...
class FrameWorker(QThread):
    ...
    def _get_scaled_pixmap(self, pixmap):
        ...
        if (
            ...
            and hasattr(self, "_cached_pixmap")
            and self._cached_size == label_size
...
class FrameViewer(QWidget):
    ...
    def _initViewLayout(self):
        ...
        for i in range(self.cameras_amount):
            ...
            camera_label.setStyleSheet(
                f"font-weight: bold; color: {self.colors['text_primary']};"
            )
            ...
            image_label.setStyleSheet(
                f"border: 1px solid {self.colors['border']}; background-color: black;"
            )
    ...
    def _initControlLayout(self):
        ...
        self.video_direction_group.buttonClicked.connect(
            self._on_video_direction_changed
        )
    ...
    def _on_video_direction_changed(self, button):
        ...
        direction = (
            VideoDirection.backward if button_id == 0 else VideoDirection.forward
        )
        for worker in self.frame_workers:
    ...
    def exportVideo(self):
        ...
        for i in cameras_to_export:
            ...
            if not images or len(images) == 0:
                QMessageBox.warning(
                    self, "Export Error", f"No images available for Camera {i+1}"
                )
                continue
            ...
            thread = Thread(
                target=save_video, args=(images, fps, output_file), daemon=True
            )
            thread.start()
